...
contract StrategyMasterchef is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant rewardTokenAddress =
        0xcF664087a5bB0237a0BAd6742852ec6c8d69A27a;
    ...
    function earn() external nonReentrant whenNotPaused onlyGov {
        ...
        if (wethAmt > 0) {
            wethAmt = distributeFees(wethAmt);
            wethAmt = distributeRewards(wethAmt);
            wethAmt = buyBack(wethAmt, wethAddress);

            if (wethAddress != token0Address) {
                // Swap hald earned to token0
                _safeSwap(
                    wethAmt.div(2),
                    wethToToken0Path,
                    address(this),
    ...
    function distributeRewards(uint256 _earnedAmt) internal returns (uint256) {
        if (rewardRate > 0) {
            ...
            uint256 rewardTokenBefore = IERC20(rewardTokenAddress).balanceOf(
                address(this)
            );
            ...
            uint256 rewardTokenAfter = IERC20(rewardTokenAddress)
                .balanceOf(address(this))
                .sub(rewardTokenBefore);
    ...
    {
        if (buyBackRate > 0) {
            // Calculate buyBackAmt
            uint256 buyBackAmt = _earnedAmt.mul(buyBackRate).div(feeMax);

            if (_earnedAddress == earnedAddress) {
                uint256 wethAmtBefore = IERC20(wethAddress).balanceOf(
                    address(this)
                );
                ...
                uint256 wethBalanceAfter = IERC20(wethAddress).balanceOf(
                    address(this)
                );
                buyBackAmt = wethBalanceAfter.sub(wethAmtBefore);
            }
    ...
    function vaultSharesTotal() public view returns (uint256) {
        (uint256 amount, ) = IMasterchef(masterchefAddress).userInfo(
            pid,
            address(this)
        );
        return amount;
